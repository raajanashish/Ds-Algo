...
public class TreeTreversal {
	...
	public static Node buildTreeFromPreOrder(Integer[] inorder, int inStart, int inEnd, Integer[] preOrder,
		...
		currentNode.left = buildTreeFromPreOrder(inorder, inStart, rootIndex - 1, preOrder, preStart + 1, preStart
				+ leftSubTreeSize);
	...
	public static Node buildTreeFromPostOrder(Integer[] inorder, int inStart, int inEnd, Integer[] postOrder,
		...
		int leftSubTreeSize = rootIndex - inStart;
		int rightSubTreSize = inEnd - rootIndex;
		currentNode.left = buildTreeFromPostOrder(inorder, inStart, rootIndex - 1, postOrder, postStart, postStart
				+ leftSubTreeSize - 1);
		currentNode.right = buildTreeFromPostOrder(inorder, rootIndex + 1, inEnd, postOrder, postStart
				+ leftSubTreeSize, postEnd - 1);
		return currentNode;
	}
	...
	public static void main(String[] args) {
		...
		Node newRoot = getBinaryTreeFromPostOrder(inrder.toArray(new Integer[inrder.size()]),
				preorder.toArray(new Integer[preorder.size()]));
		inrder.clear();
		preorder.clear();
		inorder(newRoot, inrder);
		System.out.println(inrder);
		postorder(newRoot, preorder);
		System.out.println(preorder);
